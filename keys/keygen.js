...
function sha256hex(str) {
  ...
  return elliptic.utils.sha256(utf8); // hex string
}

function pubkeyToAddress(pubkeyHex) {
  const pubBytes = new TextEncoder().encode(pubkeyHex);
  const hashHex = elliptic.utils.sha256(pubBytes); // hex string
  return hashHex.slice(0, 16); // first 8 bytes (16 hex chars)
}
